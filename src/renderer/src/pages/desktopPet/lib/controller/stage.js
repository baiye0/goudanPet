...
export class UBaseStageController extends UBaseController {
  ...
  init() {
    ...
    this.wrapper.classList.add(
      this.live2dData.fixed ? 'live2d-fixed' : 'live2d-relative',
      'live2d-wrapper',
  ...
  async _fade(element = null, proceed, exit) {
    ...
    // 添加过度类
    !element.classList.contains('live2d-transition-all') &&
      element.classList.add('live2d-transition-all')
    // 执行分支
  ...
  showAndHiddenMenus(event) {
    if (
      ...
      (event.type === 'touchstart' && this.wrapper.contains(event.touches[0].target))
    ) {
